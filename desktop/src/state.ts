...
export type CoreMem  = { id:number; fact:string; category?:string };
export type User     = { username:string; role:string; email?:string; avatar?:string; allowed_categories?:string };
export type Attach   = { filename:string; url:string; type:string; extracted_text:string|null; tempId?:string; size?:number; status?: "uploading" | "indexing" | "ready" | "failed" };
