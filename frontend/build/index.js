...
function App() {
    ...
    async function sendChat() {
        ...
        setChatMessages((prev) => [
            ...
            {
                role: "assistant",
                text: String(data.response || data.reply || data.message || "No response"),
            },
        ]);
    }
...
function Root() {
    ...
    const [serverConfigured, setServerConfigured] = React.useState(() => !isDesktopApp() || !!getServerUrl());
    if (!serverConfigured) {
        return _jsx(ServerConnect, { onConnected: () => setServerConfigured(true) });
    }
    return _jsx(App, {});
}
const rootEl = document.getElementById("root");
if (rootEl)
    createRoot(rootEl).render(_jsx(Root, {}));
