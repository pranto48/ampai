...
import { isDesktopApp, getServerUrl } from "./api-client";
  ...
  return (
    <div style={{ ...styles.card, position: "sticky", top: 10, zIndex: 2 }}>
      <div style={styles.row}>
        ...
        <button
          style={{ ...styles.btn, ...styles.s, marginLeft: "auto" }}
          onClick={logout}
          type="button"
        >
          Logout
...
function App() {
  ...
  const [username, setUsername] = useState(
    localStorage.getItem("ampai_username") || "",
  );
  ...
  async function onRegister() {
    ...
    setOk("");
    try {
      await callAuth("/api/auth/register", {
        username: username.trim(),
        password,
      });
  ...
  async function sendChat() {
    ...
    if (!res.ok) {
      setChatMessages((prev) => [
        ...prev,
        { role: "assistant", text: `Error: ${data.detail || "chat failed"}` },
      ]);
      return;
    }
    setChatMessages((prev) => [
      ...prev,
      {
        role: "assistant",
        text: String(
          data.response || data.reply || data.message || "No response",
        ),
      },
    ]);
  }
  ...
  async function loadAdminUsers() {
    ...
    const data = await decodeResponse<{ users?: AdminUser[]; detail?: string }>(
      res,
    );
      ...
      <div style={styles.root}>
        <div style={styles.container}>
          <div style={styles.card}>
            <h1>AmpAI Login</h1>
            <p>
              Default admin: <code>admin</code> / <code>P@ssw0rd</code> (legacy{" "}
              <code>admin123</code> also accepted).
            </p>
      ...
      <div style={styles.container}>
        ...
        {route === "dashboard" && (
          <div style={styles.card}>
            ...
            <p>
              Welcome,{" "}
              {me?.username || localStorage.getItem("ampai_username") || "user"}{" "}
              ({role}).
            </p>
            <ul>
              <li>
                Chat route active: <code>#/chat</code>
              </li>
              <li>
                AI model connection page: <code>#/models</code>
              </li>
              <li>
                Admin panel: <code>#/admin</code> (admin only)
              </li>
            </ul>
          </div>
        )}
          ...
          <div style={styles.card}>
            ...
            >
              {chatMessages.length === 0 && (
                <p style={{ margin: 0 }}>Start chatting with your assistant.</p>
              )}
            ...
            <ul>
              {(modelOptions.providers || []).map((p) => (
                <li key={p.value}>
                  ...
                  {(modelOptions.models?.[p.value] || []).join(", ") ||
        ...
        {ok && (
          <div style={styles.card}>
            <p style={{ color: "#86efac", margin: 0 }}>{ok}</p>
          </div>
        )}
      </div>
    </div>
 ...
 * Root
 *
 * Top-level orchestrator that decides whether to show the first-run Docker
 * server setup screen (Tauri only, when no URL is stored) or jump straight
 * into the main App.
 *
 * State flow:
 *   Tauri + no stored URL  →  <ServerConnect>  →  onConnected()  →  <App>
 *   Tauri + URL present    →  <App> directly
 *   Browser                →  <App> directly
 */
function Root() {
  // Synchronously compute initial state so there is no flicker.
  const [serverConfigured, setServerConfigured] = React.useState(
    () => !isDesktopApp() || !!getServerUrl(),
  );

  if (!serverConfigured) {
    return <ServerConnect onConnected={() => setServerConfigured(true)} />;
  }
