...
</div>`}function G(){return e.auth?`<div class="panel">
...
</div>`}function Y(){const t=[...new Set(e.sessions.map(l=>l.category||"Uncategorized"))],a=e.sessionSearch.toLowerCase(),n=[...e.sessions.filter(l=>{const r=(l.title||l.category||"").toLowerCase(),u=(l.category||"").toLowerCase(),v=l.session_id.toLowerCase(),h=!a||r.includes(a)||u.includes(a)||v.includes(a),f=!e.sessionCategoryFilter||l.category===e.sessionCategoryFilter;return h&&f})].sort((l,r)=>l.pinned&&!r.pinned?-1:!l.pinned&&r.pinned?1:(r.updated_at||"").localeCompare(l.updated_at||"")),i=e.sessionError?`<div class="session-error-banner">${o(e.sessionError)}</div>`:"";return`<div class="panel" style="padding-bottom:8px">
...
<div class="sessions-list" id="sessions-list-scroll">
  ${n.length?n.map(l=>{const r=l.title||l.category||"Untitled Chat",u=e.sessionId===l.session_id,v=l.pinned,h=e.renamingSessionId===l.session_id,f=e.assigningCategorySessionId===l.session_id;return h?`<div class="session-item active" data-sid="${o(l.session_id)}">
  ...
  </div>`:f?`<div class="session-item active" data-sid="${o(l.session_id)}">
    <div class="session-rename-form">
...
</div>`}function Q(){const t=["core","inbox","analytics"].map(s=>`<button class="sub-tab-btn${e.memSubTab===s?" active":""}" data-mem-sub="${s}">${s==="core"?"🧠 Core":s==="inbox"?"📬 Inbox":"📊 Analytics"}</button>`).join("");let a="";if(e.memSubTab==="core")a=`<div class="panel">
...
<button id="btn-reload-analytics" style="width:100%">🔄 Refresh Analytics</button>`}return`<div class="sub-tabs">${t}</div>${a}`}function X(){return`<div class="panel">
  <div class="panel-title" style="display:flex;justify-content:space-between;align-items:center">
...
${e.personaModal?Z():""}`}function Z(){const t=e.editingPersona;return`<div class="modal-overlay" id="persona-modal-overlay">
  <div class="modal-box">
...
</div>`}function ee(){if(e.auth?.role!=="admin")return'<div class="section-empty">🛡️ Admin access required.</div>';const t=e.configs;return`<div class="panel">
  ...
  <form class="stack" id="cfg-model-form">
    <label class="field">Default Provider
      <select name="default_model_provider">
        ${(e.providers.length?e.providers:A.map(s=>({value:s.value,label:s.label}))).map(s=>`<option value="${o(s.value)}"${t.default_model_provider===s.value?" selected":""}>${o(s.label)}</option>`).join("")}
      </select>
    </label>
    <label class="field">Default Model<input name="default_model" value="${o(t.default_model||"")}" placeholder="e.g. llama3.2, gpt-4o"/></label>
    <label class="field">AI Agent Name<input name="chat_agent_name" value="${o(t.chat_agent_name||"AmpAI")}" placeholder="AmpAI"/></label>
    <div class="divider"></div>
    <div class="panel-title">API Keys &amp; Endpoints</div>
    <label class="field">Ollama URL<input name="ollama_base_url" value="${o(t.ollama_base_url||"")}" placeholder="http://host.docker.internal:11434"/>
    </label>
    ...
    <label class="field">OpenRouter Key<input name="openrouter_api_key" value="${o(t.openrouter_api_key||"")}" type="password" placeholder="sk-or-…"/></label>
    <label class="field">OpenAI Key<input name="openai_api_key" value="${o(t.openai_api_key||"")}" type="password" placeholder="sk-…"/></label>
    <label class="field">Gemini Key<input name="gemini_api_key" value="${o(t.gemini_api_key||"")}" type="password"/></label>
    <label class="field">Anthropic Key<input name="anthropic_api_key" value="${o(t.anthropic_api_key||"")}" type="password"/></label>
    <label class="field">Groq Key<input name="groq_api_key" value="${o(t.groq_api_key||"")}" type="password" placeholder="gsk_…"/></label>
    <label class="field">Mistral Key<input name="mistral_api_key" value="${o(t.mistral_api_key||"")}" type="password"/></label>
    <label class="field">Cohere Key<input name="cohere_api_key" value="${o(t.cohere_api_key||"")}" type="password"/></label>
    <label class="field">LM Studio / Generic URL<input name="generic_base_url" value="${o(t.generic_base_url||"")}" placeholder="http://localhost:1234"/></label>
    <label class="field">Generic API Key<input name="generic_api_key" value="${o(t.generic_api_key||"")}" type="password"/></label>
    <label class="field">AnythingLLM URL<input name="anythingllm_base_url" value="${o(t.anythingllm_base_url||"")}" placeholder="http://localhost:3001"/></label>
    <label class="field">AnythingLLM Key<input name="anythingllm_api_key" value="${o(t.anythingllm_api_key||"")}" type="password"/></label>
    <label class="field">AnythingLLM Workspace<input name="anythingllm_workspace" value="${o(t.anythingllm_workspace||"")}" placeholder="my-workspace"/></label>
    <div class="divider"></div>
    <div class="panel-title">Memory Defaults</div>
    <label class="field">Memory Mode<select name="memory_mode">
      ${["full","indexed","context_only","none"].map(s=>`<option${t.memory_mode===s?" selected":""}>${s}</option>`).join("")}
    </select></label>
    <label class="field">Memory Top-K<input name="memory_top_k" value="${o(t.memory_top_k||"5")}" type="number" min="1" max="30"/></label>
    <label class="field">SerpAPI Key (Web Search)<input name="serpapi_api_key" value="${o(t.serpapi_api_key||"")}" type="password"/></label>
...
<div class="panel">
  ...
  <div class="theme-swatches">
    ${H.map(t=>`<div class="theme-swatch${e.themeAccent===t.value?" active":""}" data-accent="${t.value}" style="background:${t.value}" title="${t.name}"></div>`).join("")}
  </div>
...
<div class="panel">
...
</div>`}return`<div class="sub-tabs">${t}</div>${a}`}function se(){const t=e.updateVersion,a=e.updateStatus;return`<div class="panel">
  ...
  </div>`:`<div class="hint" style="margin-bottom:8px">Current version: <strong>v${U}</strong> — Up to date ✓</div>`}
</div>
<div class="panel">
...
`))}</div>`:""}
</div>`}function ne(){const t=e.configs,a=e.modelType||t.default_model_provider||"ollama",s=e.modelName||t.default_model||"",n=A.map(u=>{const v=a===u.value,h=u.keyField?!!(t[u.keyField]||"").trim():!0,f=e[`${u.value}Models`]||e.providerModels?.[u.value]||[],R=u.local?'<span class="badge ok" style="font-size:.65rem">Local</span>':h?'<span class="badge ok" style="font-size:.65rem">Key Set</span>':'<span class="badge bad" style="font-size:.65rem">No Key</span>';return`<div class="provider-card${v?" active":""}" data-select-provider="${o(u.value)}">
  <div class="provider-card-header">
    <span class="provider-card-label">${o(u.label)}</span>
    ${R}
  </div>
  <div class="provider-card-models">
    ${f.length?`<span style="font-size:.72rem;color:var(--muted)">${f.length} model${f.length>1?"s":""} available</span>`:`<button class="sm" data-fetch-models="${o(u.value)}" style="font-size:.72rem">🔍 Fetch Models</button>`}
  </div>
</div>`}).join(""),i=e.providerModels?.[a]||[],l=i.length?i.map(u=>{const v=s===u.id,h=u.free?'<span class="badge ok" style="font-size:.6rem;margin-left:4px">FREE</span>':"",f=u.context_length?`<span style="font-size:.68rem;color:var(--muted);margin-left:6px">${Math.round(u.context_length/1e3)}K ctx</span>`:"";return`<div class="model-item${v?" selected":""}" data-select-model="${o(u.id)}">
  <span class="model-item-name">${o(u.name||u.id)}</span>${h}${f}
...
<div class="panel">
  <div class="panel-title" style="display:flex;justify-content:space-between;align-items:center">
    Models — ${o(A.find(u=>u.value===a)?.label||a)}
...
</div>`}function ie(){const t=e.taskState.tasks,a=t.filter(r=>r.status==="todo"),s=t.filter(r=>r.status==="in_progress"),n=t.filter(r=>r.status==="done");function i(r){return`<span class="badge ${{urgent:"bad",high:"warn",medium:"info",low:"ok"}[r]||"info"}">${o(r)}</span>`}function l(r){return`<div class="task-card">
...
</div>`}function re(){const t=e.terminalState,a=t.enabled?'<span class="badge ok">Enabled</span>':'<span class="badge bad">Disabled</span>',s=t.policy,n=`<div class="terminal-policy-grid">
...
</div>`}function M(t){const a=(t||"").trim();if(!a)return e.serverUrl;const s=/^https?:\/\//i.test(a)?a:`http://${a}`;try{return new URL(s).origin}catch{return e.serverUrl}}function B(t){e.auth=t,t?localStorage.setItem(E,JSON.stringify(t)):localStorage.removeItem(E)}function y(){return e.auth?.role==="admin"}function de(t){const a=new Headers(t);return a.has("Content-Type")||a.set("Content-Type","application/json"),e.auth?.token&&a.set("Authorization",`Bearer ${e.auth.token}`),a}async function c(t,a={}){const s=new AbortController,n=setTimeout(()=>s.abort(),25e3);try{const i=await fetch(`${e.serverUrl}${t}`,{...a,headers:de(a.headers),signal:s.signal}),l=await i.text(),r=l?JSON.parse(l):{};if(!i.ok)throw new Error(r?.detail||r?.message||i.statusText);return r}finally{clearTimeout(n)}}function T(t){const a=document.documentElement;a.style.setProperty("--accent",t),a.style.setProperty("--accent-2",t),e.themeAccent=t,localStorage.setItem(j,t)}function ce(){const t=document.querySelector(".sidebar");t&&t.classList.toggle("collapsed",!!e.sidebarCollapsed)}function d(t,a="info"){const s=document.getElementById("toast-container");if(!s)return;const n=document.createElement("div");n.className=`toast toast-${a}`,n.textContent=t,s.appendChild(n),setTimeout(()=>n.remove(),3500)}function $(t,a){e.msgs.push({role:t,content:a,time:new Date().toLocaleTimeString()})}function L(){const t=globalThis.crypto?.randomUUID?.()||`d-${Date.now()}-${Math.random().toString(16).slice(2)}`;return e.sessionId=t,localStorage.setItem(I,t),t}function me(){return e.attachments.map((t,a)=>`
...
</div>`).join("")}function ue(){if(!e.msgs.length)return`
...
</div>`;const t=e.msgs.map(a=>{const s=e.auth?.username?.[0]?.toUpperCase()||"U",n=a.role==="user"?s:a.role==="system"?"i":"AI",i=a.role==="user"?o(a.content):`<div>${W(a.content)}</div>`;return`
...
</div>`),t.join("")}function m(){const t=document.getElementById("app");if(!t)return;t.innerHTML=`
<div class="app-shell">
  <aside class="sidebar${e.sidebarCollapsed?" collapsed":""}">
    <div class="sidebar-header">
      ...
      <button class="collapse-btn" id="btn-sidebar-collapse">${e.sidebarCollapsed?"»":"«"}</button>
    </div>
    <div class="tab-bar">
      ${b("server","💬 Chat")}
      ${b("account","Account")}
      ${e.auth?b("history","History"):""}
      ${e.auth?b("memory","🧠 Memory"):""}
      ${e.auth?b("ai","🤖 AI Models"):""}
      ${e.auth?b("tasks","📋 Tasks"):""}
      ${e.auth?b("browser","🌐 Browser"):""}
      ${e.auth?b("terminal","⌨️ Terminal"):""}
      ${e.auth?b("personas","AI Personas"):""}
      ${e.auth?b("settings","⚙️ Settings"):""}
      ${e.auth?b("personalise","🎨 Personalise"):""}
      ${y()?b("telegram","📱 Telegram"):""}
      ${y()?b("admin","🛡️ Admin"):""}
      ${y()?b("update","Update"):""}
    </div>
    <div class="tab-panels">
      ...
      ${e.auth?g("tasks",ie()):""}
      ${e.auth?g("browser",le()):""}
      ${e.auth?g("terminal",re()):""}
      ${e.auth?g("personas",X()):""}
      ${e.auth?g("settings",ee()):""}
      ${e.auth?g("personalise",te()):""}
      ${y()?g("telegram",ve()):""}
      ${y()?g("admin",ae()):""}
      ${y()?g("update",se()):""}
    </div>
  ...
  <main class="chat-shell">
    ${pe()}
    <div class="chat-messages" id="msgs">${ue()}</div>
    <div class="chat-input-bar">
      <div class="attach-pills" id="attach-pills">${me()}</div>
      <div class="input-box">
...
<div id="toast-container"></div>`;const a=document.getElementById("msgs");a&&(a.scrollTop=a.scrollHeight),ce(),ye()}function b(t,a){return`<button class="tab-btn${e.tab===t?" active":""}" data-tab="${t}">${o(a)}</button>`}function g(t,a){return`<div class="tab-panel${e.tab===t?" active":""}">${a}</div>`}function pe(){const t=e.providers.length?e.providers:[{value:"ollama",label:"Ollama"},{value:"generic",label:"LM Studio"},{value:"anythingllm",label:"AnythingLLM"},{value:"openrouter",label:"OpenRouter"},{value:"openai",label:"OpenAI"},{value:"gemini",label:"Gemini"},{value:"anthropic",label:"Anthropic"}],a=e.sessions.find(i=>i.session_id===e.sessionId),s=e.providerModels[e.modelType]||[],n=s.length?s.map(i=>`<option value="${o(i.id)}"${e.modelName===i.id?" selected":""}>${o(i.name||i.id)}${i.free?" ✦":""}</option>`).join(""):`<option value="${o(e.modelName)}">${o(e.modelName||"default")}</option>`;return`
...
</div>`}function ve(){const t=e.configs,a=e.tgStatus;return`<div class="panel">
