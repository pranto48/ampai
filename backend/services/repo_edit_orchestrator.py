...
class RepoEditOrchestrator:
    def __init__(
        self,
        github_token: str,
        planner: Optional[Callable[[str, RepoTargetContext, Dict[str, str]], EditPlan]] = None,
    ...
    def _build_plan(self, instruction: str, context: RepoTargetContext, snapshot: Dict[str, str], job_id: str) -> EditPlan:
        if self.planner:
            return self.planner(instruction, context, snapshot)

        payload = self._with_retry_timeout(
            lambda: self.provider.planEdits(instruction, snapshot, job_id=job_id),
            operation="planEdits",
        )
        files_to_modify = payload.get("files_to_modify") or list(snapshot.keys())
        patch_hunks: List[PatchHunk] = []
        for file_path in files_to_modify:
            if file_path in snapshot:
                generated = self._with_retry_timeout(
                    lambda fp=file_path: self.provider.generatePatch({fp: snapshot[fp]}, instruction, job_id=job_id),
                    operation="generatePatch",
                )
                patch_hunks.append(PatchHunk(file_path=file_path, content=generated))
        changes = {h.file_path: h.content for h in patch_hunks}
        summary = self._with_retry_timeout(
            lambda: self.provider.summarizePR(changes, job_id=job_id),
            operation="summarizePR",
        ) if changes else "No changes generated."
        return EditPlan(
            files_to_modify=files_to_modify,
            patch_hunks=patch_hunks,
            rationale=payload.get("rationale", "Generated by provider."),
            summary=summary,
            risk_notes="Automated provider output; review required.",
        )

    def _with_retry_timeout(self, action: Callable[[], Any], operation: str) -> Any:
        attempts = max(1, int(self.provider.budget.max_retries) + 1)
        timeout = float(self.provider.budget.timeout_seconds)
        ...
        for _ in range(attempts):
            ...
            try:
                ...
                if elapsed > timeout:
                    raise TimeoutError(f"{operation} exceeded timeout ({elapsed:.2f}s > {timeout:.2f}s)")
                return result
            except (ProviderError, TimeoutError, ValueError, RuntimeError) as exc:
                last_error = exc
        raise RuntimeError(f"Provider {operation} failed after {attempts} attempts: {last_error}")
