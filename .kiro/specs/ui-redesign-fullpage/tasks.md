...
- [x] 1. Rewrite Docker updater to use archive-only path
  - [x] 1.1 Rewrite `_do_update_in_thread()` in `main.py` to remove the git fetch/reset path
...
- [x] 2. Add missing providers to backend model fetch endpoint
  - [x] 2.1 Add groq, mistral, and cohere support to `GET /api/models/fetch/{provider}` in `routers/models_router.py`
...
- [x] 4. Fix chat topbar model selector to use ALL_PROVIDERS
  - [x] 4.1 Update `chatTopbar()` in `desktop/src/main.ts` to use `ALL_PROVIDERS` for the provider dropdown
  ...
  - [x] 4.2 Fix provider change handler in `bind()` to always fetch models and re-render
...
- [x] 5. Fix AI Models page provider and model fetching
  - [x] 5.1 Update `aiTab()` in `desktop/src/tabs-ai.ts` to wire fetch-on-provider-select
  ...
  - [x] 5.2 Add AI tab event bindings in `bindAI()` in `desktop/src/main.ts`
...
- [x] 6. Checkpoint - Ensure model selector works end-to-end
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. Clean up render() for full-page navigation
  - [x] 7.1 Remove legacy sidebar code from `render()` in `desktop/src/main.ts`
  ...
  - [x] 7.2 Ensure nav bar renders correctly with active state highlighting
...
- [x] 8. Update CSS for full-page layout
  - [x] 8.1 Review and clean up `desktop/src/styles.css` for page-based layout
...
- [x] 9. Wire update UI feedback with polling
  - [x] 9.1 Update `bindUpdate()` in `desktop/src/main.ts` to implement status polling
